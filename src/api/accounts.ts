...
function normalizeByokKeys(
	apiKeys: unknown,
	legacyApiKey?: string,
): Array<{ label: string; key: string; weight?: number; priority?: number }> {
	...
	for (const [index, item] of rawKeys.entries()) {
		const label = String(item.label || `key-${index + 1}`)
			.trim()
			.toLowerCase();
		...
		if (!BYOK_KEY_LABEL_RE.test(label)) {
			throw new Error(
				"key label must start with lowercase alphanumeric and contain only lowercase letters, numbers, hyphen, or underscore",
			);
		}
...
function normalizeByokLbMethod(
	value: unknown,
): "round_robin" | "sequential" | "least_inflight" {
	return value === "sequential" || value === "least_inflight"
		? value
		: "round_robin";
}
...
async function getByokLbMethods(
	prefixes: string[],
...
/**
 * Check if a credential (API key, JWT, PAT, password) already exists for the
 * given provider. XOR encryption is deterministic, so we compare encrypted
 * values directly without decrypting all rows.
 *
 * @returns The existing account row if a duplicate is found, null otherwise.
 */
async function findDuplicateCredential(
	credential: string,
	provider: string,
	excludeId?: number,
	...
	return (
		rows.find((a) => a.password === encrypted && a.id !== (excludeId ?? -1)) ??
		null
	);
}

function duplicateError(existing: typeof accounts.$inferSelect): {
	error: string;
	duplicate: { id: number; email: string; provider: string };
} {
	return {
		error: `Duplicate credential detected — this key/token is already used by account #${existing.id} (${existing.email})`,
		duplicate: {
			id: existing.id,
...
accountsRouter.post("/byok", async (c) => {
	...
	if (!BYOK_PREFIX_RE.test(label)) {
		return c.json(
			{ error: "label must be lowercase alphanumeric with hyphens only" },
			400,
		);
	}
	...
	if (existingByok.some((acc) => getByokPrefix(acc) === label)) {
		return c.json(
			{ error: "BYOK provider with this label already exists" },
			409,
		);
	}

	// Check for duplicate keys across existing BYOK accounts
	for (const keyInput of keyInputs) {
		const dup = await findDuplicateCredential(keyInput.key, "byok");
		if (dup) return c.json(duplicateError(dup), 409);
	}
	...
	try {
		...
		for (const [index, keyInput] of keyInputs.entries()) {
			const tokens: ByokTokensShape = {
				...
				load_balancing_method: normalizeByokLbMethod(
					body.load_balancing_method,
				),
			};
		...
		await setByokLbMethod(
			label,
			normalizeByokLbMethod(body.load_balancing_method),
		);
...
accountsRouter.get("/byok", async (c) => {
	...
	const lbMethods = await getByokLbMethods(
		Array.from(new Set(byokAccounts.map((acc) => getByokPrefix(acc)))),
	);
		...
		if (!existing) {
			groups.set(prefix, {
				...
				load_balancing_method:
					lbMethods.get(prefix) ||
					tokens.load_balancing_method ||
					"round_robin",
				keys: [],
			});
		} else {
			...
			existing.status =
				existing.status === "active" || acc.status !== "active"
					? existing.status
					: "active";
		}
...
accountsRouter.post("/byok/:id/reveal", async (c) => {
	...
	if (!Number.isFinite(id))
		return c.json({ error: "Invalid BYOK key id" }, 400);
	...
	try {
		...
		return c.json(
			{
				error:
					error instanceof Error ? error.message : "Failed to decrypt BYOK key",
			},
			500,
		);
	}
	...
	try {
		...
		if (keyPayloadProvided) {
			...
			for (const [index, keyInput] of desiredKeys.entries()) {
				const keyLabel = String(keyInput.label || `key-${index + 1}`)
					.trim()
					.toLowerCase();
				...
				if (seenLabels.has(keyLabel))
					return c.json(
						{ error: `duplicate BYOK key label: ${keyLabel}` },
						400,
					);
				...
				const existing = groupAccounts.find(
					(acc) =>
						(keyInput.id && acc.id === keyInput.id) ||
						getByokKeyLabel(acc) === keyLabel,
				);
				const tokens: ByokTokensShape = {
					...
					headers: nextHeaders,
					key_label: keyLabel,
					weight: Number.isFinite(Number(keyInput.weight))
						? Number(keyInput.weight)
						: undefined,
					priority: Number.isFinite(Number(keyInput.priority))
						? Number(keyInput.priority)
						: index,
					load_balancing_method: normalizeByokLbMethod(
						body.load_balancing_method || currentTokens.load_balancing_method,
					),
				};
				...
				if (existing) {
					const updateData: Record<string, unknown> = {
						...
						tokens,
						enabled:
							typeof keyInput.enabled === "boolean"
								? keyInput.enabled
								: existing.enabled,
						updatedAt: new Date(),
					};
					...
					if (!keySecret)
						return c.json(
							{ error: `new key "${keyLabel}" requires a secret` },
							400,
						);
					// Duplicate key check for newly added keys
					const dupNew = await findDuplicateCredential(keySecret, "byok");
					if (dupNew) return c.json(duplicateError(dupNew), 409);
			...
			for (const acc of toDelete) {
				await db
					.update(requestLogs)
					.set({ accountId: null })
					.where(eq(requestLogs.accountId, acc.id));
			...
			for (const acc of groupAccounts) {
				...
				const updateData: Record<string, unknown> = {
					tokens: {
						...
						headers: nextHeaders,
						load_balancing_method: normalizeByokLbMethod(
							body.load_balancing_method || tokens.load_balancing_method,
						),
					},
					updatedAt: new Date(),
				};
				if (body.api_key && acc.id === id)
...
accountsRouter.post("/byok/:id/test", async (c) => {
	...
	const isAnthropic =
		format === "anthropic" ||
		(format === "auto" &&
			(tokens.base_url.includes("anthropic.com") ||
				tokens.base_url.includes("/v1/messages")));
...
export async function createGitlabDuoAccount(
	...
	// Duplicate PAT check (skip if updating an existing account)
	if (!input.existingAccountId) {
		const dupPat = await findDuplicateCredential(pat, "gitlab-duo");
		if (dupPat) {
			return {
				ok: false,
				status: 409,
				error: `Duplicate credential — this PAT is already used by account #${dupPat.id} (${dupPat.email})`,
			};
		}
	}
	...
	try {
		const r = await fetch(`${baseUrl}/api/v4/personal_access_tokens/self`, {
			headers,
		});
		if (!r.ok)
			return {
				ok: false,
				status: 400,
				error: `PAT invalid (HTTP ${r.status})`,
			};
		...
		return {
			ok: false,
			status: 502,
			error: `Cannot reach GitLab: ${e instanceof Error ? e.message : String(e)}`,
		};
	}
	...
	try {
		...
		if (json.errors)
			return {
				ok: false,
				status: 400,
				error: `GraphQL: ${JSON.stringify(json.errors)}`,
			};
		...
		if (!cu)
			return {
				ok: false,
				status: 400,
				error: "currentUser is null — PAT lacks read_user scope?",
			};
		...
		if (!ns) {
			return {
				...
				status: 400,
				error:
					"Cannot resolve a namespace for this PAT. Either set a default namespace in GitLab → Preferences → Duo, or grant the user access to at least one group.",
			};
		}
		...
		return {
			ok: false,
			status: 502,
			error: `GraphQL fetch failed: ${e instanceof Error ? e.message : String(e)}`,
		};
	}
	...
	try {
		...
		if (r.ok) {
			...
			const nodes: Array<{
				id?: string;
				username?: string;
				usage?: { creditsUsed?: number; totalCredits?: number };
			...
			const endDate = trial?.activeTrial?.endDate
				? new Date(trial.activeTrial.endDate)
				: null;
	...
	try {
		if (input.existingAccountId) {
			...
			if (!row)
				return {
					ok: false,
					status: 404,
					error: "Pending account row not found",
				};
			pool.invalidate("gitlab-duo" as ProviderName);
			const { refreshGitlabDuoModels } =
				await import("../proxy/providers/registry");
		...
		if (existing) {
			return {
				ok: false,
				status: 409,
				error: "GitLab Duo account with this label already exists",
			};
		}
...
accountsRouter.post("/gitlab-duo/:id/refresh", async (c) => {
	...
	const tokens = (
		typeof account.tokens === "string"
			? JSON.parse(account.tokens)
			: account.tokens
	) as { gitlabBaseUrl: string; namespaceId?: number };
	const oldMeta =
	...
	try {
		...
		const ns = duoNs ?? fallbackNs;
		if (!ns)
			return c.json({ error: "no namespace resolvable for this PAT" }, 400);
		...
		const gitlabVersion =
			modelsJson.data?.metadata?.version ?? oldMeta.gitlabVersion ?? "";

		const nextTokens = { ...tokens, namespaceId, namespacePath };
		const nextMeta = {
			...oldMeta,
			defaultModel:
				dm?.ref ?? oldMeta.defaultModel ?? "claude_sonnet_4_6_vertex",
		...
		try {
			...
			if (duoProvider) {
				...
				if (probe.success && probe.quota && probe.quota.limit >= 0) {
					...
					if (probe.quota.resetAt instanceof Date)
						quotaResetAt = probe.quota.resetAt;
				}
			}
		} catch {
...
accountsRouter.get("/:id/models", async (c) => {
	...
	if (!provider)
		return c.json({ error: `Unknown provider: ${account.provider}` }, 400);
...
accountsRouter.post("/:id/test", async (c) => {
	...
	if (!provider) {
		return c.json(
			{ success: false, error: `Unknown provider: ${account.provider}` },
			400,
		);
	}
	...
	if (!models || models.length === 0) {
		return c.json(
			{ success: false, error: "Provider has no supported models" },
			400,
		);
	}
	...
	if (reqBody.model) {
		...
		if (!found) {
			return c.json(
				{
					success: false,
					error: `Model "${reqBody.model}" not available for provider ${account.provider}`,
				},
				400,
			);
		}
	...
	try {
		...
		return c.json({
			success: false,
			error:
				stillExhausted && account.status === "exhausted"
...
accountsRouter.post("/:id/reveal", async (c) => {
	...
	if (account.provider !== "byok" && account.provider !== "codebuddy-china") {
		return c.json({
			success: false,
			error:
				"This account type does not expose a key/token (uses email+password auth)",
...
accountsRouter.post("/", async (c) => {
	...
	if (body.provider === "qoder" && body.personalToken) {
		...
		// Duplicate PAT check — Qoder stores PAT as encrypt("pat-login") in password,
		// but the real PAT ends up in tokens.access_token after activation. We check
		// the raw PAT input against existing passwords encrypted with the PAT value.
		const dupQoder = await findDuplicateCredential(trimmed, "qoder");
		if (dupQoder) return c.json(duplicateError(dupQoder), 409);
		...
		try {
			...
			const email =
				jobToken.email ||
				jobToken.name ||
				`qoder-${tokens.userId || Date.now()}@pat`;
			...
			if (existing) {
				...
				broadcast({
					type: "account_updated",
					data: { id: existing.id, provider: "qoder", status: "active" },
				});
				return c.json(
					{
						id: existing.id,
			...
			broadcast({
				type: "account_created",
				data: { id: created.id, provider: "qoder", email },
			});
	...
	if (body.provider === "youmind" && body.apiKey) {
		...
		// Duplicate API key check
		const dupYm = await findDuplicateCredential(trimmed, "youmind");
		if (dupYm) return c.json(duplicateError(dupYm), 409);
		...
		try {
			...
			if (existing) {
				...
				broadcast({
					type: "account_updated",
					data: { id: existing.id, provider: "youmind", status: "active" },
				});
				return c.json(
					{
						id: existing.id,
			...
			broadcast({
				type: "account_created",
				data: { id: created.id, provider: "youmind", email },
			});
			return c.json({ ...created, password: "***", tokens: null }, 201);
		} catch (error) {
			const msg = error instanceof Error ? error.message : String(error);
			return c.json(
				{ error: `YouMind API key activation failed: ${msg}` },
				400,
			);
		}
	}
	...
	if (body.provider === "codebuddy-china" && body.apiKeys) {
		...
		// Validate format + duplicate check
		for (const key of keys) {
			if (!key.startsWith("ck_")) {
				return c.json(
					{
						error: `Invalid API key format: ${key.substring(0, 20)}... (must start with ck_)`,
					},
					400,
				);
			}
			const dupCk = await findDuplicateCredential(key, "codebuddy-china");
			if (dupCk) return c.json(duplicateError(dupCk), 409);
		}
		...
		broadcast({
			type: "account_created",
			data: { provider: "codebuddy-china", count: created.length },
		});
	...
	if (body.provider === "codebuddy-china" && body.jwtResponse) {
		...
		if (
			...
			return c.json(
				{
					error:
						"jwtResponse missing a valid access_token (expected a JWT starting with eyJ)",
				},
				400,
			);
		}
		...
		// Duplicate JWT check
		const dupJwt = await findDuplicateCredential(
			accessToken,
			"codebuddy-china",
		);
		if (dupJwt) return c.json(duplicateError(dupJwt), 409);
		...
		const existingCount = await db
			...
			.then((rows) => rows.length);
		const email =
			parsed?.identity?.username || uid || `cbc-jwt-${existingCount + 1}`;
		...
		broadcast({
			type: "account_created",
			data: { provider: "codebuddy-china", id: inserted[0]?.id },
		});
	...
	// Duplicate password/credential check for generic providers
	const dupGeneric = await findDuplicateCredential(
		body.password,
		body.provider,
	);
	if (dupGeneric) return c.json(duplicateError(dupGeneric), 409);
	...
	try {
		...
		return c.json(
			{
				...created,
				password: "***",
				tokens: created.tokens ? "[set]" : null,
				loginQueued: true,
			},
			201,
		);
		...
		if (
			...
			return c.json(
				{ error: "Account with this email already exists for this provider" },
				409,
			);
		}
		throw error;
...
accountsRouter.post("/instant-login", async (c) => {
	...
	for (const refreshToken of body.tokens) {
		...
		try {
			...
			if (!response.ok) {
				errors.push(
					`token ...${trimmed.slice(-8)}: refresh failed (${response.status})`,
				);
				failed++;
				continue;
			}
			...
			errors.push(
				`token ...${trimmed.slice(-8)}: ${err instanceof Error ? err.message : String(err)}`,
			);
			failed++;
		}
	}
	...
	if (success > 0) {
		broadcast({
			...
			data: { provider: "kiro-pro", count: success },
		});
	}
...
accountsRouter.post("/bulk", async (c) => {
	...
	for (const acc of body.accounts) {
		try {
			...
			const dupBulk = await findDuplicateCredential(acc.password, acc.provider);
			if (dupBulk) {
				results.push({
					email: acc.email,
					success: false,
					error: `Duplicate credential — already used by account #${dupBulk.id} (${dupBulk.email})`,
				});
				continue;
			}
			await db.insert(accounts).values({
...
accountsRouter.patch("/:id", async (c) => {
	...
	if (body.quotaRemaining !== undefined)
		updateData.quotaRemaining = body.quotaRemaining;
	...
	return c.json({
		...updated,
		password: "***",
		tokens: updated.tokens ? "[set]" : null,
	});
});
...
accountsRouter.post("/toggle-all", async (c) => {
	...
	const count = await pool.setEnabledByProvider(
...
accountsRouter.post("/bulk-delete", async (c) => {
	const body = await c.req
		...
		.catch(() => ({}) as { ids?: Array<number | string> });
	...
	if (ids.length === 0) {
		return c.json(
			{ error: "ids must be a non-empty array of account ids" },
			400,
		);
	}
	...
	await db
		.update(requestLogs)
		.set({ accountId: null })
		.where(inArray(requestLogs.accountId, foundIds));
	await db
		.update(vccCards)
		.set({ usedByAccountId: null })
		.where(inArray(vccCards.usedByAccountId, foundIds));
	await db
		.delete(vccTransactions)
		.where(inArray(vccTransactions.accountId, foundIds));

	const result = await db
		.delete(accounts)
		.where(inArray(accounts.id, foundIds))
		.returning();
	...
	broadcast({
		...
		data: { ids: deletedIds, providers: providersAffected },
	});
...
accountsRouter.delete("/:id", async (c) => {
	...
	await db
		.update(requestLogs)
		.set({ accountId: null })
		.where(eq(requestLogs.accountId, id));
	await db
		.update(vccCards)
		.set({ usedByAccountId: null })
		.where(eq(vccCards.usedByAccountId, id));
...
export async function importCodexAccessToken(
	...
	let email = String(
		profileClaim?.email || claims.email || claims.preferred_username || "",
	);
	...
	const planType = String(
		authClaim?.chatgpt_plan_type || claims.plan_type || "",
	);
	...
	if (!email || !accountId) {
		try {
			...
			if (usageResp.ok) {
				...
				if (!accountId)
					accountId = String(
						usage.account_id || usage.chatgpt_account_id || "",
					);
			}
		} catch {}
	}
...
export async function exchangeCodexRefreshTokens(tokens: string[]) {
	...
	for (const refreshToken of tokens) {
		...
		try {
			...
			if (!response.ok) {
				const text = await response.text().catch(() => "");
				errors.push(
					`token ...${trimmed.slice(-8)}: refresh failed (${response.status}): ${text.slice(0, 100)}`,
				);
				failed++;
				continue;
			}
			...
			if (!data.access_token) {
				errors.push(
					`token ...${trimmed.slice(-8)}: no access_token in response`,
				);
				failed++;
				continue;
			}
	...
	if (success > 0) {
		broadcast({
			type: "accounts_updated",
			data: { provider: "codex", count: success },
		});
	}
...
accountsRouter.post("/byok", async (c) => {
	...
	if (
		...
		return c.json(
			{ error: "label, base_url, api_key, and models[] are required" },
			400,
		);
	}
...
accountsRouter.get("/byok", async (c) => {
	...
	const providers = byokAccounts.map((acc) => {
		const tokens =
			typeof acc.tokens === "string" ? JSON.parse(acc.tokens) : acc.tokens;
		...
		return {
			...
			status: acc.status,
			enabled: acc.enabled,
			available_models: (tokens?.models || []).map(
				(m: string) => `${tokens?.model_prefix || acc.email}-${m}`,
			),
		};
	});
...
accountsRouter.patch("/byok/:id", async (c) => {
	...
	return c.json({
		...
		id,
		label: account.email,
		models: (tokens.models || []).map(
			(m: string) => `${tokens.model_prefix || account.email}-${m}`,
		),
	});
});
...
accountsRouter.post("/:id/open-panel", async (c) => {
	...
	try {
		...
		if (account.provider.startsWith("kiro")) {
			...
			const profileArn =
				tokens.profile_arn || tokens.profileArn || refreshData.profileArn || "";
			...
			if (!webCookie) {
				await browser.close();
				return c.json(
					{ error: "No web_cookie available for Qoder account" },
					400,
				);
			}
			...
			if (qoderCookies.length === 0) {
				await browser.close();
				return c.json(
					{ error: "No valid Qoder cookies found in web_cookie" },
					400,
				);
			}
